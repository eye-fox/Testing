...
def build_android(working_dir, build_type):
    ...
    if build_type == "1":
        ...
        process = subprocess.Popen("./gradlew assembleDebug --parallel", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        ...
        process = subprocess.Popen("./gradlew assembleRelease --parallel", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        for line in process.stdout:
            print(line, end='')
        process.wait()
        result = process.returncode
